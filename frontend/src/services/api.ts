...
export async function publishToS3(sessionId: string): Promise<{ url: string; project_name: string }> {
  return request<{ url: string; project_name: string }>(`/export/${sessionId}/publish`, {
    method: 'POST',
  });
}
