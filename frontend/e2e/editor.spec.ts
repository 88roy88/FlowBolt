...
function trackUploadRequests(page: import('@playwright/test').Page) {
  ...
  page.on('request', (req) => {
    if (
      req.method() === 'POST' &&
      req.url().includes('/api/files/') &&
      req.url().includes('/project_file/upload')
    ) {
      uploadCount += 1;
    }
  });
...
test.describe('Editor', () => {
  ...
  test('can create a new file inside a folder', async ({ page }) => {
    ...
    const createReq = page.waitForRequest(
      (req) => req.method() === 'POST' && req.url().includes('/api/files/') && req.url().includes('/project_file'),
      { timeout: 10_000 }
    );
  ...
  test('can upload an asset from the file tree', async ({ page }) => {
    ...
    const uploadReq = page.waitForRequest(
      (req) => req.method() === 'POST' && req.url().includes('/api/files/') && req.url().includes('/project_file/upload'),
      { timeout: 10_000 }
    );
  ...
  test('can rename a folder from the file tree', async ({ page }) => {
    ...
    const renameReq = page.waitForRequest(
      (req) => req.method() === 'PATCH' && req.url().includes('/api/files/') && req.url().includes('/project_file'),
      { timeout: 10_000 }
    );
  ...
  test('can delete a file from the file tree', async ({ page }) => {
    ...
    const deleteReq = page.waitForRequest(
      (req) => req.method() === 'DELETE' && req.url().includes('/api/files/') && req.url().includes('/project_file'),
      { timeout: 10_000 }
    );
