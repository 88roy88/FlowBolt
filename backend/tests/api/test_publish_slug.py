...
def _mock_project(published_url: str = "", published_slug: str | None = None):
    proj = AsyncMock()
    proj.published_url = published_url
    proj.published_slug = published_slug
    return proj
...
class TestSlugCheck:
    ...
    async def test_taken_slug_returns_unavailable(self):
        with patch("flow44.api.publish.is_slug_taken", return_value=True):
    ...
    async def test_free_slug_returns_available(self):
        with patch("flow44.api.publish.is_slug_taken", return_value=False):
...
class TestPublish:
    ...
    async def test_publish_with_slug(self):
        ...
        with (
            ...
            p_deploy,
            p_bucket,
            patch("flow44.api.publish.is_slug_taken", return_value=False),
            patch("flow44.api.publish.update_project_published_url") as mock_update,
        ):
            ...
            assert response.status_code == 200
            data = response.json()
            assert data["url"] == "/api/share/my-cool-app"
            assert data["slug"] == "my-cool-app"
            mock_update.assert_awaited_once_with("proj-1", MOCK_S3_URL, slug="my-cool-app")
    ...
    async def test_publish_without_slug(self):
        ...
        with (
            ...
            assert response.status_code == 200
            data = response.json()
            assert data["url"] == "/api/export/proj-1/published"
            assert data["slug"] == ""
            mock_update.assert_awaited_once_with("proj-1", MOCK_S3_URL, slug=None)
    ...
    async def test_publish_with_taken_slug_returns_409(self):
        ...
        with (
            ...
            p_deploy,
            p_bucket,
            patch("flow44.api.publish.is_slug_taken", return_value=True),
        ):
            response = client.post(
                "/api/export/proj-1/publish",
    ...
    async def test_republish_same_slug_succeeds(self):
        ...
        with (
            ...
            p_bucket,
            patch("flow44.api.publish.is_slug_taken", return_value=False),
            patch("flow44.api.publish.update_project_published_url"),
        ):
            response = client.post(
                "/api/export/proj-1/publish",
            ...
            assert response.status_code == 200
            assert response.json()["url"] == "/api/share/existing-slug"
...
class TestShareBySlug:
    ...
    async def test_share_returns_proxied_html(self):
        mock_proj = _mock_project(published_url="https://s3.local/published.html")
        with patch("flow44.api.share.get_project_by_slug", return_value=mock_proj):
            with patch("httpx.AsyncClient.get") as mock_get:
                ...
                response = client.get("/api/share/my-app")
    ...
    async def test_share_unknown_slug_returns_404(self):
        with patch("flow44.api.share.get_project_by_slug", return_value=None):
