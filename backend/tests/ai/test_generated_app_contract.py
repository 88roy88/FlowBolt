...
@pytest.mark.parametrize(
    ...
    [
        ...
        "src/main.tsx",
        "src/vite-env.d.ts",
        "src/platform/react-router-dom/routerBasename.ts",
        "backend/server.py",
    ],
)
...
def test_unselected_external_import_is_rejected() -> None:
    ...
    with pytest.raises(GeneratedAppContractError, match="react-router-dom"):
...
"""
    ...
    assert disallowed_external_imports(content, ["react-router-dom"]) == set()
    assert assert_generated_code_contract("src/App.tsx", content, ["react-router-dom"]) == "src/App.tsx"


def test_dynamic_unselected_import_is_rejected() -> None:
    assert disallowed_external_imports("const page = import('react-router-dom');", []) == {"react-router-dom"}
