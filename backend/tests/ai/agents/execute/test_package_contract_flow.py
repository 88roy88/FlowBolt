...
from .test_optional_packages import CONNECTED_ASSETS_PROMPT
...
@pytest.mark.asyncio
async def test_connected_assets_packages_are_installed_before_merge(monkeypatch: pytest.MonkeyPatch) -> None:
    sandbox = FakeSandbox()
    build_state = BuildState(project_id="project", user_content=CONNECTED_ASSETS_PROMPT)
    ...
    async def fake_complete_chat(
        ...
        assert sandbox.install_calls == [["react-router-dom", "regraph"]]
        assert "`react-router-dom` is selected" in system_prompt
        assert "`regraph` is selected" in system_prompt
        calls.append("merge")
        return """
        {
...
async def test_selected_packages_are_confirmed_before_codegen(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    plan = WorkPlan(
        ...
        selected_packages=["react-router-dom", "regraph"],
    )
