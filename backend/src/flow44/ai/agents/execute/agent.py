...
class ExecuteAgent(BaseAgent):
    ...
    async def _seed_routing_files(self, state: ExecutionState) -> None:
        for path in ("src/router/AppRouter.tsx", "src/utils/routerBasename.ts"):
            content = await state.sandbox_ref.read_file(path)
            state.build_state.completed_files[path] = content

    def _routing_dependency_files(self, state: ExecutionState) -> dict[str, str]:
        paths = ("src/router/AppRouter.tsx", "src/utils/routerBasename.ts")
        return {path: state.build_state.completed_files[path] for path in paths if path in state.build_state.completed_files}
